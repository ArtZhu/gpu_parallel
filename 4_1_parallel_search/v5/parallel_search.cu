...
__global__ void search_main(number * X, int n, number target, int num_threads, volatile int * dev_ret, int dev_ret_len, int * ret_value, int * flags)
{
	extern __shared__ int found[];

	int jmp = 2 + blockDim.x;
